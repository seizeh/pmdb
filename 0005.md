...
**파일**: `supabase/migrations/20260525120005_rls_and_views.sql`
**카테고리**: 초기 세트업
**의존성**: 0001~0004

## 목적

전 26개 테이블 RLS 활성화 + 61개 정책(SELECT/INSERT/UPDATE/DELETE)을 걸어 행 단위 접근 제어를 완성합니다. public_profiles 뷰로 민감 컬럼(password_hash, email 등)을 안전 컬럼만 노출.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| Schema/Table Grants | anon: SELECT 전체 + post_views INSERT / authenticated: 쓰기 전체 / service_role: 전권 |
| 헬퍼 | app.is_room_member() SECURITY DEFINER — 채팅 RLS 재귀 방지 |
| 정책 요약 | 본인·admin 원칙 + 게시글/사용자는 공개 조회, 채팅/알림은 관계자만 |
| public_profiles VIEW | id, username, nickname, user_type, profile_image, address, is_location_verified, created_at 만 노출 |
...
- 0006 에서 public_profiles 를 security_invoker 로 재정의하며 users 정책도 함께 조정됨.
- post_views 는 익명 조회수 기록을 위해 anon INSERT 허용 (특별 GRANT).
- reviews INSERT 는 policy + 트리거 이중 검증 (당사자·완료·중복 방지).
