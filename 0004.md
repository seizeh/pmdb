...
**파일**: `supabase/migrations/20260525120004_triggers.sql`
**카테고리**: 초기 세트업
**의존성**: 0002, 0003

## 목적

상태 전이 검증, 카운트 캐시 동기화, 지원-수락-약속 자동 생성, 채팅 last_message 유지 등 비즈니스 로직 안전망 37개 트리거를 정의합니다. cross-row 변경 함수는 SECURITY DEFINER + search_path='' 로 RLS 우회.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| 공통 | updated_at 자동 갱신 트리거 13개 |
| 게시글 | deleted_at 동기화, 상태전이 검증, 작성권한 체크, 카운트 캐시(like/comment/view) |
| 지원·약속 | 신청 시 차단 검사, 수락 시 약속 생성(이중 수락 방지 SELECT FOR UPDATE), 완료·취소 동기화 |
| 평가 | 당사자 검증, 카테고리 중복 차단, review_category_counts 집계 |
| 채팅 | last_message 갱신, 미읽음 카운트, 소프트 삭제 preview 처리, 읽음 처리 시 unread 감소 |
| 알림 | unread_notification_count 캐시 |
| 가입 후 | notification_preferences + admin 문의방 자동 생성 |
| 감사 | admin 액션 자동 로그 (posts/comments/reports) |
...
- app.tg_applications_on_accept: 게시글 lock → recruiting 확인 → matched 갱신(RETURNING) → 약속 INSERT. 이중 수락은 partial unique(scheduled 1개)로 최종 방어.
- app.tg_chat_members_read: last_read_message_id 갱신 시 (created_at, id) 튜플 비교로 정확한 unread 감소.
- 미읽음 카운트는 캐시라 drift 가능 → 0014 에서 reconcile RPC 추가.
