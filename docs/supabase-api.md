...
- **응답**:
  ...
  - 200 실패 `{ enrolled: false, reason: "ai_unavailable" | "not_real_pet"(+ai) | "not_consistent_pet"(+ai) | "species_mismatch"(+ai) }` — `frames_not_from_video`는 **섀도 모드**(판정 false 시 실패 기록만 남기고 등록은 통과, `FRAMES_FROM_VIDEO_ENFORCE=true` 전환 시 거절 사유로 활성화)
...
- **정책**: 라이브니스 하한 0.70(`ENROLL_REAL_THRESHOLD`), 프레임 최소 3장. 프레임↔영상 동일 출처(`frames_from_video`)는 2026-07-29부터 **섀도 모드**(challenge 를 오탐으로 제거한 전례에 따라, `photo_verifications`의 `fail_reason='frames_not_from_video'` 비율로 오탐률 측정 후 하드 게이트 전환 — 클라 프레임 추출 시각과 Gemini 1fps 샘플링 어긋남이 오탐 소지). 인라인 페이로드 합계 한도 19M chars(`video_too_large`, Gemini 요청당 20MB 보호선). Gemini 429 시 1.5s/3s backoff 최대 2회 재시도.
