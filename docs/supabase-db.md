...
| `retention-purge` | `23 3 * * *` (매일 03:23) | `app.cleanup_retention()` — 전화 인증코드 1일 / 위치 인증 이력·사진 인증 로그 6개월 경과분 파기. photo_verifications 는 pets·posts FK 참조 때문에 **미참조 행만 삭제, 참조 행은 촬영 좌표(shot_*)만 스크럽** (→ §13 `20260711130000`) |
...
| `20260711130000` | `retention_purge_batch` | `app.cleanup_retention()` + pg_cron `retention-purge`(매일 03:23): phone_verifications 1일, location_verifications 6개월, photo_verifications 6개월 자동 파기. photo_verifications 는 pets/posts FK 참조로 미참조 행만 삭제·참조 행은 촬영 좌표만 스크럽(FK 위반→잡 전체 롤백 방지). |
