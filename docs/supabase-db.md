...
- `share_view_load(p_token) → jsonb` — 링크 검증(`not_found`/`expired`/`ok`) + kind 별 본문 조회 + 계측(view_count 원자 증가, funnel `share_view`)을 단일 왕복으로. `facility_preview` 는 시설 요약 + 최근 후기 3건(visible 만). app 스키마가 PostgREST 미노출이라 Edge Function 도 이 RPC 를 경유한다.
