...
**파일**: `supabase/migrations/20260525120012_block_free_post_applications.sql`
**카테고리**: 정책
**의존성**: 0009

## 목적

도메인 문서 4-1 은 자유(free) 카테고리에 '평가 시스템 N' 으로 명시하나 DB 는 자유 글에도 지원→수락→약속→평가 흐름이 통과. 이 모순을 applications BEFORE INSERT 트리거에서 원천 차단.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| tg_applications_block_insert | category='free' 검사 추가 → 지원 시 예외 발생 |
...
- 지원이 막히면 후속 단계(수락/약속/평가)는 자연히 도달 불가.
- 자유 글은 토론·정보 공유 용도 — 매칭이 아닌 커뮤니티 게시글.
