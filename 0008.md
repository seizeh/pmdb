# 0008 · 반려동물 N:M 공동보호자 구조

**파일**: `supabase/migrations/20260525120008_pet_guardians.sql`
**카테고리**: 기능 변경
**의존성**: 0007

## 목적

pets:users 1:N → N:M 으로 확장. 같은 반려동물을 여러 사용자가 공유(가족 공동양육)할 수 있도록 pet_guardians 중간 테이블을 도입. pet_guardian_invites 로 초대·요청 흐름을 제공.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| pets 컬럼 리네임 | user_id → primary_guardian_id (최고 권한 owner) |
| 신규 테이블 | pet_guardians (pet_id, user_id, role=owner|co_guardian, invited_by) — 펫당 owner 1명 partial unique |
| 신규 테이블 | pet_guardian_invites (kind=invite|request, inviter_id, invitee_phone/user_id, status) |
| 트리거 | app.is_pet_guardian() 헬퍼, 펫 등록 시 owner 자동 생성, 초대 수락 시 co_guardian 추가 |
| 가입 확장 | 가입 시 이미 와 있던 초대(phone 매칭)를 invitee_user_id 로 자동 연결 |
| 작성 권한 | 게시글 카테고리별 owner/guardian 검증으로 재정의 |
| post_pets 검증 | 남의 펫 붙이기 차단, 분양은 owner + 1마리 |
| RLS | pets/pet_guardians/invites 전면 재정의 (guardian 기반) |
...
- 0009 에서 이 마이그레이션의 분양(give_away) 완료 시 이전 시맨틱이 '기존 owner 강등' → '기존 보호자 전원 정리' 로 갱신됨.
- 권한 결정: owner 전용(펫 수정·삭제·분양·초대/제거·이전) / co_guardian(펫 조회·글 작성·글에 펫 연결).
- app.is_pet_guardian 은 SQL 언어라 CREATE 시 본문 파싱 → pet_guardians 테이블 생성 이후에 정의해야 함(현재 파일 순서에 반영).
