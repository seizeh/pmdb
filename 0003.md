...
**파일**: `supabase/migrations/20260525120003_indexes.sql`
**카테고리**: 초기 세트업
**의존성**: 0002

## 목적

성능 크리티컬 경로의 46개 인덱스를 생성합니다. lower() 유일성 인덱스, 부분 unique(race-safe 중복 방지), trigram GIN(부분검색), 복합 인덱스(목록/지도/동 단위 필터) 등.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| 유일성 | lower(username), lower(email), lower(nickname) — 대소문자 무관 중복 차단 |
| Partial unique | post_views(view_bucket), appointments(scheduled 상태 1개), notifications(unread 병합) |
| Trigram GIN | posts(title+content) — 게시글 부분검색 |
| 복합 인덱스 | posts(visibility, progress, created_at), (region_code, progress, created_at) |
| 좌표 인덱스 | posts(display_lat, display_lng) — 지도 박스 검색 (MVP 임시) |
...
- post_views 는 partial unique 두 개: (post_id, user_id, view_bucket)WHERE user_id NOT NULL, (post_id, ip_hash, view_bucket)WHERE ip_hash NOT NULL — race condition 없이 중복 조회 차단.
- notifications 는 partial unique + 미읽음 조회용 index 두 개 → ON CONFLICT DO UPDATE 병합 패턴.
- MVP 지도 검색은 박스, 확장 시 PostGIS + GIST 전환.
