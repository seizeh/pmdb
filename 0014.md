# 0014 · 탈퇴자 표시 + unread 보정 + 정책 코멘트

**파일**: `supabase/migrations/20260525120014_inactive_user_visibility_and_unread_reconcile.sql`
**카테고리**: 하드닝
**의존성**: 0005~0013

## 목적

탈퇴(status='inactive') 사용자의 안전 컬럼(닉네임/프로필 이미지)이 RLS 로 차단돼 과거 콘텐츠 JOIN 시 NULL 로 깨지는 실제 결함 수정. 미읽음 카운트 drift 복구용 RPC 추가. 운영 정책(신고 orphan, 알림 결국정확 등)을 COMMENT 로 명시.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| users RLS 완화 | status <> 'suspended' 인 사용자는 공개 조회 허용 (inactive 포함, suspended 만 차단) |
| RPC | app.reconcile_unread_counts(user_id) — chat/notification 미읽음 캐시를 source-of-truth 로 재계산 |
| COMMENT | post_likes/pawings hard-delete 정책, pet_guardian_invites partial unique, applications 재지원 불가, reports orphan, notifications 결국정확 |
...
- reconcile RPC 는 앱 진입/재연결/다중기기 동기화 직후 호출 권장.
- 탈퇴자의 민감 컬럼은 컬럼 GRANT(0006) 로 여전히 차단됨 — 닉네임/프로필 이미지만 공개.
- 'suspended' 는 관리자 정지/금지 → UI 에서 '차단된 사용자' 로 표시.
