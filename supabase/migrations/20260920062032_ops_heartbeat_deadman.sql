-- pg_cron dead man's switch — 감시 체인이 자기 장애 영역 밖으로 박동을 보낸다
...
-- 감시할 수는 없으므로 방향을 뒤집는다: 5분마다 밖(healthchecks.io)으로 박동을
-- 보내고, 박동이 grace 내에 끊기면 **외부**가 운영자에게 알린다.
--
-- 엣지 함수만 죽는 경우(DB 생존)는 이 박동이 못 본다 — 별도 층(외부 프로브) 과제.
--
-- ping URL 은 capability 다(아는 사람이 가짜 박동으로 죽음을 가릴 수 있다) —
...
comment on table app.heartbeat_config is
  'dead man''s switch 박동 대상(healthchecks.io) 싱글턴 — 값은 out-of-band 주입';
