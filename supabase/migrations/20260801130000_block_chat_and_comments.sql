...
-- ── 4) block_user 수정 — reports 유니크 충돌로 차단이 실패하던 것 ────────
--
-- reports 에는 유니크가 **둘** 있는데 처음엔 하나만 방어했다:
--   reports_one_open_per_target — 미처리 건만(부분 인덱스)
--   reports_uq                  — (reporter, target, type) **상태 무관 전체**
-- 후자 때문에 '과거에 신고했다가 처리 완료된 상대'는 INSERT 가 터지고,
-- 통보가 부가 기능인데도 **차단 자체가 실패**했다(실제로 재현됨).
