# 0013 · 입양(adoption) 자동 이전

**파일**: `supabase/migrations/20260525120013_adoption_transfer.sql`
**카테고리**: 기능
**의존성**: 0009

## 목적

분양(give_away)과 방향이 반대인 입양 흐름을 자동 이전으로 완성. 글 작성자=받는 사람(adopter), 지원자=주는 사람(giver). applications.offered_pet_id 로 지원자가 넘길 펫을 명시하고, 완료 시 지원자→작성자로 이전.

## 핵심 변경

| 구분 | 내용 |
|---|---|
| applications 컬럼 | offered_pet_id UUID REFERENCES pets(id) — 입양 시 필수, 그 외 NULL |
| 검증 트리거 | 입양 글 지원 시 offered_pet_id 필수 + 신청자가 owner + 펫 active 검증 |
| 불변 가드 | trg_applications_immutable_offer — 지원 후 offered_pet_id 변경 차단 |
| 펫 점유 확장 | 0009 의 conflict 쿼리를 offered_pet_id 까지 포함하도록 확장 (2중 방어) |
| 이전 로직 | 입양 완료 시 offered_pet 의 pet_guardians 전원 정리 → 작성자만 owner |
...
- 분양(give_away): 글에 붙은 pet 이 작성자→지원자로. 입양(adoption): application.offered_pet 이 지원자→작성자로.
- 펫 점유 검사가 두 방향(post_pets ∪ offered_pet_id) 모두 고려하도록 확장됨.
